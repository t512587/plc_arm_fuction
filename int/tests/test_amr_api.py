...
class FakeApiAmrService:
    ...
    def goto(self, position_id, is_reverse, nav_type, *, poll_interval=0.35):
        self.goto_calls.append((position_id, is_reverse, nav_type, poll_interval))
        return {
            "success": True,
            "arrived": True,
...
class AmrApiTests(unittest.TestCase):
    ...
    def test_cancel_task_endpoint_does_not_disconnect(self) -> None:
        fake = FakeApiAmrService()

        with patch.object(api_main, "AMR_NAVIGATION_SERVICE", fake):
            response = api_main.cancel_amr_current_task()

        self.assertTrue(response.ok)
        self.assertEqual(1, fake.cancel_task_calls)
        self.assertEqual(0, fake.cleanup_calls)

    def test_confirm_status_endpoint(self) -> None:
        fake = FakeApiAmrService()

        with patch.object(api_main, "AMR_NAVIGATION_SERVICE", fake):
            response = api_main.confirm_amr_status()

        self.assertTrue(response.ok)
        self.assertEqual(1, fake.confirm_status_calls)
