...
@app.post("/amr/cancel-task", response_model=APIResponse)
def cancel_amr_current_task() -> APIResponse:
    try:
        return APIResponse(
            ok=True,
            data=AMR_NAVIGATION_SERVICE.cancel_task(),
        )
    except Exception as exc:  # noqa: BLE001
        return _amr_failure("cancel_task", exc)


@app.post("/amr/confirm-status", response_model=APIResponse)
def confirm_amr_status() -> APIResponse:
    try:
        return APIResponse(
            ok=True,
            data=AMR_NAVIGATION_SERVICE.confirm_status(),
        )
    except Exception as exc:  # noqa: BLE001
        return _amr_failure("confirm_status", exc)
