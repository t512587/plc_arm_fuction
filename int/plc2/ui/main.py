...
AMR_TASK_TYPES = {
    "amr_connect", "amr_setobs", "amr_read_map", "amr_goto",
    "amr_cancel_task", "amr_confirm_status", "amr_disconnect",
}
FLOW_TERMINAL_STATUSES = {"success", "cancelled", "timeout", "error"}
HOME_FLOW_REQUEST_TIMEOUT_SECONDS = 210
# AMR arrival waiting has no server-side deadline (amr_cmd.action.goto_amr_position
# polls until the AMR FSM explicitly reports succeeded/failed). This is only a
# socket-level backstop so the HTTP client does not block forever; it is not a
# navigation timeout. Use the existing "Cancel AMR" control to abort a stuck task.
AMR_GOTO_HTTP_TIMEOUT_SECONDS = 3600
        ...
        if step.type == "amr_goto":
            return [(
                ...
                "/amr/goto",
                {
                    ...
                    "is_reverse": step.values["is_reverse"] == "true",
                    "nav_type": int(step.values["nav_type"]),
                    "poll_interval": float(step.values["poll_interval"]),
                },
                AMR_GOTO_HTTP_TIMEOUT_SECONDS,
            )]
        if step.type == "amr_cancel_task":
            return [(
                f"TXT step {step.index} AMR cancel task",
                "/amr/cancel-task",
                None,
                10,
            )]
        if step.type == "amr_confirm_status":
            return [(
                f"TXT step {step.index} AMR confirm status",
                "/amr/confirm-status",
                None,
                10,
            )]
    ...
    def _task_step_summary(step: TaskStep) -> str:
        ...
        if step.type == "amr_goto":
            return (
                ...
                f"is_reverse={step.values.get('is_reverse')} "
                f"nav_type={step.values.get('nav_type')} "
                f"poll={step.values.get('poll_interval')}s"
            )
        if step.type == "amr_cancel_task":
            return f"{step.index}. AMR cancel task"
        if step.type == "amr_confirm_status":
            return f"{step.index}. AMR confirm status"
        if step.type == "amr_disconnect":
