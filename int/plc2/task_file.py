...
def _validate_step(step: TaskStep) -> None:
    if step.type == "control_settings":
        ...
        "amr_connect", "amr_disconnect", "amr_cancel_task", "amr_confirm_status",
    }:
        ...
        _reject_unknown_keys(
            step,
            {"type", "position_id", "is_reverse", "nav_type", "poll_interval"},
        )
        step.values["position_id"] = _required(step, "position_id")
        step.values["is_reverse"] = "true" if _bool(step, "is_reverse") else "false"
        nav_type = _whole_number(step, "nav_type")
        if nav_type not in {1, 2}:
            raise TaskFileError(
                f"step {step.index}: nav_type must be 1 (自動導航) or 2 (路徑導航)"
            )
        step.values["nav_type"] = str(nav_type)
...
def _bool(step: TaskStep, key: str) -> bool:
    value = _required(step, key).strip().casefold()
    if value in {"true", "1"}:
        return True
    if value in {"false", "0"}:
        return False
    raise TaskFileError(f"step {step.index}: {key} must be true or false")
